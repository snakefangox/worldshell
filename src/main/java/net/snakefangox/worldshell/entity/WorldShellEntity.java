    ...
    protected ActionResult handleInteraction(PlayerEntity player, Hand hand, boolean interact) {
        ...
        if (rayCastResult.getType() == HitResult.Type.BLOCK) {
            if (interact) {
                ...
                // TODO: Find and use some vanilla method for this
                BlockState state = microcosm.getBlockState(rayCastResult.getBlockPos());
                ActionResult actionResult = ActionResult.PASS;

                if (state.getBlock() instanceof ShellAwareBlock)
                    actionResult = ((ShellAwareBlock) state.getBlock()).onUseInShell(microcosm.getProxyWorld(), this, player, hand, rayCastResult);

                if (actionResult == ActionResult.PASS)
                    actionResult = state.onUse(microcosm.getProxyWorld(), player, hand, rayCastResult);

                if (actionResult == ActionResult.PASS) {
                    ItemStack stack = player.getStackInHand(hand);
                    actionResult = stack.useOnBlock(new ItemUsageContext(microcosm.getProxyWorld(), player, hand, stack, rayCastResult));
                }
